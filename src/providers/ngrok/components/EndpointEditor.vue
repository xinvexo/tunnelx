<template>
  <TxDrawer
    ...
    :title="source ? t('providerForm.editEndpoint') : t('providerForm.createEndpoint')"
  ...
  >
    <SettingsGroup v-if="draft" :title="t('providerForm.basic')">
      ...
      <SettingsRow :label="t('providerForm.localAddress')">
        <input v-model="draft.addr" class="tx-input mono" placeholder="http://localhost:8080" />
      </SettingsRow>
      <SettingsRow :label="t('providerForm.domain')">
        <input v-model="draft.domain" class="tx-input mono" :placeholder="t('providerForm.optional')" />
      </SettingsRow>
...
import { newNgrokEndpoint, type NgrokEndpoint } from '../domain'

const props = defineProps<{ modelValue: boolean; source: NgrokEndpoint | null }>()
const emit = defineEmits<{ 'update:modelValue': [boolean]; save: [NgrokEndpoint] }>()
const ui = useUiStore()
const { t } = useI18n()
const draft = ref<NgrokEndpoint | null>(null)
const protoOptions = [
...
watch(() => props.modelValue, (open) => {
  if (!open) return
  draft.value = props.source ? clone(props.source) : newNgrokEndpoint()
})
...
function submit() {
  ...
  draft.value.domain = draft.value.domain.trim()
