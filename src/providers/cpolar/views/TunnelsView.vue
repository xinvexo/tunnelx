...
const metadata = computed(() => resource.value ? cpolarMetadata(resource.value) : null)
...
const emptyHint = computed(() => hasCredentials.value ? '' : t('providerTunnel.credentialsRequired', { credential: 'cpolar Authtoken' }))
const editorOpen = ref(false)
const editing = ref<CpolarEndpoint | null>(null)
...
function openNew() {
  if (!hasCredentials.value) {
    ui.notify(t('providerTunnel.credentialsRequired', { credential: 'cpolar Authtoken' }), 'warning')
    return
  }
  editing.value = null
...
function openEdit(endpoint: CpolarEndpoint) {
...
async function saveMetadata(next: CpolarMetadata, successMessage?: string) {
  if (!resource.value) return
  try {
    await connections.update(applyCpolarMetadata(resource.value, next))
...
function reorderEndpoints(ids: string[]) {
  ...
  const nextEndpoints = ids.map((item) => byId.get(item)).filter((endpoint): endpoint is CpolarEndpoint => !!endpoint)
  if (nextEndpoints.length !== endpoints.value.length) return
  void saveMetadata({ ...metadata.value, endpoints: nextEndpoints })
}

function saveEndpoint(endpoint: CpolarEndpoint) {
  if (!metadata.value) return
  ...
  void saveMetadata({ ...metadata.value, endpoints: nextEndpoints }, t('providerTunnel.saved', { provider: 'cpolar' }))
}

async function toggleEndpoint(endpoint: CpolarEndpoint, enabled: boolean) {
  if (!metadata.value) return
...
async function removeEndpoint(endpoint: CpolarEndpoint) {
  if (!metadata.value) return
  ...
  }, t('providerTunnel.removed', { provider: 'cpolar' }))
}
