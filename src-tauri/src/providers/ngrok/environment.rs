...
use crate::providers::contract::{ProviderRuntimeUpdateStatus, ProviderStatus, NGROK_PROVIDER_ID};
...
fn begin_environment_operation() -> AppResult<EnvironmentOperationGuard> {
    ENVIRONMENT_OPERATION_BUSY
        ...
        .map_err(|_| AppError::Msg("ngrok runtime operation is already in progress".into()))
}
...
pub fn resolve_command<R: Runtime>(app: &AppHandle<R>) -> AppResult<String> {
    ...
    if managed.exists() {
        ...
        Err(AppError::Msg(
            "ngrok runtime is not installed. Install it from Runtime first".into(),
        ))
    }
}
    ...
    let (available, version, message) = match version_output {
        Some(output) if output.status.success() => {
            ...
            (
                ...
                "ngrok runtime is installed".into(),
            )
        }
        Some(output) => {
            let text = command_output_text(&output);
            (
                ...
                if text.is_empty() {
                    "ngrok runtime is unavailable".into()
                } else {
                    text
                },
            )
        }
        None => (false, None, "ngrok runtime is not installed".into()),
    };
    ProviderStatus {
...
pub async fn install(app: AppHandle, state: AppState) -> AppResult<ProviderStatus> {
    ...
    command.arg("version");
    if let Err(error) = validate_executable(&mut command, "ngrok") {
        let _ = tokio::fs::remove_file(&exe_tmp).await;
        return Err(error);
    }
...
pub fn check_update(app: &AppHandle, state: &AppState) -> ProviderRuntimeUpdateStatus {
    ...
    ProviderRuntimeUpdateStatus {
        provider_id: NGROK_PROVIDER_ID.into(),
        runtime: "ngrok".into(),
        current_version: current.version.as_deref().and_then(extract_version),
        latest_version: None,
        update_available: false,
        message: "ngrok does not expose a side-effect-free stable update check".into(),
    }
}
...
fn ensure_not_running(state: &AppState) -> AppResult<()> {
    let running = state
        ...
        .active_keys()
        .iter()
        .any(|(provider_id, _)| provider_id == NGROK_PROVIDER_ID);
    if running {
        Err(AppError::Msg("Stop running ngrok connections first".into()))
    } else {
        Ok(())
    }
}
...
fn download_url() -> AppResult<String> {
    let os = if cfg!(target_os = "macos") {
        ...
        return Err(AppError::Msg(
            "Automatic ngrok installation is not supported on this operating system".into(),
        ));
    };
    let arch = if cfg!(target_arch = "x86_64") {
        ...
        return Err(AppError::Msg(
            "Automatic ngrok installation is not supported on this CPU architecture".into(),
        ));
    };
    Ok(format!(
...
fn unpack_zip(archive: &Path, dest: &Path) -> AppResult<()> {
    ...
    Err(AppError::Msg(
        "ngrok executable was not found in the downloaded archive".into(),
    ))
}
